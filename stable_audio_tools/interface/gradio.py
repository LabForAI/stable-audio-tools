...
def generate_uncond(
    ...
    if init_audio is not None:
        ...
        if audio_length > sample_size:
            ...
            input_sample_size = (
                audio_length
                + (model.min_input_length - (audio_length % model.min_input_length))
                % model.min_input_length
            )
...
def create_autoencoder_ui(model_config):
    ...
    with gr.Blocks() as ui:
        ...
        latent_noise_slider = gr.Slider(
            minimum=0.0, maximum=10.0, step=0.001, value=0.0, label="Add latent noise"
        )
        process_button = gr.Button("Process", variant="primary", scale=1)
        process_button.click(
            fn=autoencoder_process,
            inputs=[input_audio, latent_noise_slider, n_quantizers_slider],
    ...
    with gr.Blocks() as ui:
        ...
        with gr.Row():
            temperature_slider = gr.Slider(
                minimum=0, maximum=5, step=0.01, value=1.0, label="Temperature"
            )
            top_p_slider = gr.Slider(
                minimum=0, maximum=1, step=0.01, value=0.95, label="Top p"
            )
        ...
        generate_button.click(
            fn=generate_lm,
            inputs=[temperature_slider, top_p_slider, top_k_slider],
...
def create_ui(
    ...
    assert (pretrained_name is not None) ^ (
        model_config_path is not None and ckpt_path is not None
    ...
    if model_type == "diffusion_cond" or model_type == "diffusion_cond_inpaint":
        ui = create_diffusion_cond_ui(
            model_config, model, in_model_half=model_half, gradio_title=gradio_title
        )
