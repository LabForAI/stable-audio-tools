...
class EncoderBlock(nn.Module):
    def __init__(
        ...
        self.ff = FeedForward(out_channels, out_channels)
        self.norm = nn.LayerNorm(out_channels, eps=1e-2)
    ...
    def forward(self, x):
        ...
        b, c, l = out.shape
        shortcut = rearrange(x, "b c (l h) -> b (c h) l", h=self.patch_size).view(
            b, -1, c, l
        )
        shortcut = torch.mean(shortcut, dim=1)

        return (shortcut * self.shortcut_param) + out
...
class DecoderBlock(nn.Module):
    ...
    def forward(self, x):
        ...
        _, c, _ = out.shape
        shortcut = rearrange(x, "b (c h) l -> b c (h l)", h=self.patch_size)
        nbm_duplication = c // shortcut.shape[1]
        shortcut = shortcut.repeat(1, nbm_duplication, 1)
...
class AudioAutoencoder(nn.Module):
    ...
    def encode_audio(self, audio, chunked=False, overlap=32, chunk_size=128, **kwargs):
        ...
        if not chunked:
            ...
            chunks = []
            print(
                f"Chunking audio into {chunk_size} samples chunks with {overlap} samples overlap, hop size {hop_size} samples"
            )
