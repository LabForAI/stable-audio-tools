...
from ..data.utils import VolumeNorm
from ..inference.sampling import sample, sample_discrete_euler
...
from .pretransforms import AutoencoderPretransform, Pretransform
from .transformer import ContinuousTransformer, RotaryEmbedding, TransformerBlock
            ...
            if isinstance(layer, nn.ModuleList):
                for transformer in layer:
                    if self.checkpointing:
                        ...
                        x = transformer(
                            x, self_attention_flash_sliding_window=self.sliding_window
                        )
            else:
                if self.checkpointing:
...
class AudioAutoencoder(nn.Module):
    ...
    def preprocess_audio_list_for_encoder(self, audio_list, in_sr_list):
        ...
        for i in range(batch_size):
            ...
            new_audio[i] = volume_norm(new_audio[i])
        # convert to tensor
    ...
    def encode_audio(self, audio, chunked=False, overlap=32, chunk_size=128, **kwargs):
        ...
        if not chunked:
            ...
            y_final = torch.zeros(
                (batch_size, self.latent_dim, y_size), dtype=chunks.dtype
            ).to(audio.device)
    ...
    def decode_audio(
        ...
        if not chunked:
            ...
            y_size = total_size * samples_per_latent
            y_final = torch.zeros(
                (batch_size, self.out_channels, y_size), dtype=chunks.dtype
            ).to(latents.device)
            for i in range(num_chunks):
                x_chunk = chunks[i, :]
                # decode the chunk
...
class DiffusionAutoencoder(AudioAutoencoder):
    ...
    def decode(self, latents, shape, steps=100):
        ...
        noise = torch.randn(*shape, device=latents.device)

        if self.pretransform is not None:
            if self.pretransform.enable_grad:
                noise = self.pretransform.encode(noise)
            else:
                with torch.no_grad():
                    noise = self.pretransform.encode(noise)

        decoded = sample_discrete_euler(
            self.diffusion, noise, steps, input_concat_cond=latents
        )
