...
class RunningInstanceNorm(nn.Module):
    def __init__(
        self, dim, momentum=0.99, eps=1e-4, saturate=True, trainable_gain=True
    ):
        super().__init__()
    ...
    def _update_stats(self, x):
        self.running_mean = self.running_mean * self.momentum + x.detach().mean(
            dim=[0, 1]
        ).view(1, 1, self.dim) * (1 - self.momentum)
        self.running_std = (
            self.running_std * self.momentum
            + x.detach().std(dim=[0, 1]).view(1, 1, self.dim) * (1 - self.momentum)
        ).clip(min=self.eps)
...
class LayerNorm(nn.Module):
    ...
    def forward(self, x):
        if not self.force_fp32:
            return F.layer_norm(
                x, x.shape[-1:], weight=self.gamma, bias=self.beta, eps=self.eps
            )
        else:
            output = F.layer_norm(
                x.float(),
...
class Attention(nn.Module):
    def __init__(
        ...
        if qk_norm not in ["l2", "ln", "dyt", "none"]:
            raise ValueError(
                f'qk_norm must be one of ["l2", "ln", "none"], got {qk_norm}'
            )
        ...
        if causal:
            print(
                "Using `causal` argument disables FlexAttention. If you want to use them together, incorporate causal masking into `flex_attention_block_mask`."
            )
    ...
    def apply_attn(
        ...
        if flash_attn_sliding_window is not None and (not flash_attn_available):
            print(
                f"Cannot use FlashAttention sliding window as FlashAttention is disabled or not available"
            )

        if (
            flex_attention_block_mask is not None
            or flex_attention_score_mod is not None
        ) and flash_attn_sliding_window is not None:
            print(
                f"cannot use both FlashAttention and FlexAttention, favouring FlexAttention"
            )
    ...
    def forward(
        ...
        if hasattr(self, "to_q"):
            ...
            if self.differential:
                ...
                q, q_diff = map(
                    lambda t: rearrange(t, "b n (h d) -> b h n d", h=h), (q, q_diff)
                )
                ...
                k, v = map(
                    lambda t: rearrange(t, "b n (h d) -> b h n d", h=kv_h), (k, v)
                )
        else:
...
class TransformerBlock(nn.Module):
    def __init__(
        ...
        if layer_scale and zero_init_branch_outputs:
            print(
                "zero_init_branch_outputs is redundant with layer_scale, setting zero_init_branch_outputs to False"
            )
            zero_init_branch_outputs = False

        self.pre_norm = (
            LayerNorm(dim, **norm_kwargs) if not remove_norms else DynamicTanh(dim)
        )
        ...
        **kwargs,
        ...
        if use_abs_pos_emb:
            self.pos_emb = AbsolutePositionalEmbedding(
                dim, abs_pos_emb_max_length + self.num_memory_tokens
            )
        ...
        for i in range(depth):
            should_cross_attend = cross_attend and (
                self.final_cross_attn_ix == -1 or i <= (self.final_cross_attn_ix)
            )
            self.layers.append(
                TransformerBlock(
                    dim,
