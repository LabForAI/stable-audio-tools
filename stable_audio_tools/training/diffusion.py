...
class DiffusionAutoencoderTrainingWrapper(pl.LightningModule):
    ...
    def __init__(
        ...
        if optimizer_configs is None:
            optimizer_configs = {
                "autoencoder": {"optimizer": {"type": "Adam", "config": {"lr": lr}}}
            }
        else:
            if lr is not None:
                print(
    ...
    def configure_optimizers(self):
        diffusion_opt_config = self.optimizer_configs["autoencoder"]
        opt_diff = create_optimizer_from_config(
            diffusion_opt_config["optimizer"], self.diffae.parameters()
        )
    ...
    def training_step(self, batch, batch_idx):
        ...
        log_dict = {
            ...
            "train/lr": self.trainer.optimizers[0].param_groups[0]["lr"],
            "train/gradient_norm": self.gradient_norm(self.diffae),
        }
...
class DiffusionAutoencoderDemoCallback(pl.Callback):
    ...
    def on_train_batch_end(
        ...
        with torch.no_grad() and torch.cuda.amp.autocast():
            ...
            fakes = module.diffae_ema.ema_model.decode(
                latents, encoder_input.shape, steps=self.demo_steps
            )
