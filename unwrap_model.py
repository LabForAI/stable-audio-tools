...
if __name__ == "__main__":
    ...
    if model_type == "autoencoder":
        ...
        if training_config.get("use_ema", False):
            ...
            ema_copy = create_model_from_config(model_config).to(device)
        ...
        training_wrapper = AutoencoderTrainingWrapper.load_from_checkpoint(
            ...
            ema_copy=(
                ema_copy.to(device) if (ema_copy is not None and use_ema) else None
            ),
            map_location=device,
        )
        ...
        training_wrapper = DiffusionUncondTrainingWrapper.load_from_checkpoint(
            args.ckpt_path, model=model, strict=False
        )
        ...
        training_wrapper = DiffusionAutoencoderTrainingWrapper.load_from_checkpoint(
            args.ckpt_path, model=model, ema_copy=ema_copy, strict=False
        )
